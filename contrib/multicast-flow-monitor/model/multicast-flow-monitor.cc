...
MulticastFlowMonitor::GetTypeId (void)
{
  ...
  //                  DoubleValue (0.001),
  //                  MakeDoubleAccessor (&MulticastFlowMonitor::m_delayBinWidth),
  //                  MakeDoubleChecker <double> ())
  //   .AddAttribute ("JitterBinWidth", ("The width used in the jitter histogram."),
  //                  DoubleValue (0.001),
  //                  MakeDoubleAccessor (&MulticastFlowMonitor::m_jitterBinWidth),
  //                  MakeDoubleChecker <double> ())
...
MulticastFlowMonitor::ReportRx (Ptr<MulticastFlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize, uint32_t nodeId)
{
	...
	// std::cout << "rxPackets = " << stats.rxPackets[nodeId] << std::endl;
	// std::cout << "\n";
	...
	// 	Time interArrivalTime = now - stats.timeLastRxPacket[nodeId];
	...
	{
		// std::cout << "FlowID: " << flowId << " NodeID: " << *(it) << " PacketId: " << packetId << " delivered: " << stats.groupDelivered[*(it)][packetId] << std::endl;
                         			...
                         			uint32_t nodeId)
{
	...
	// probe->AddPacketDropStats (flowId, packetSize, 7, nodeId);
