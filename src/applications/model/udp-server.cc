...
void
UdpServer::SetRxCallback (Callback<void, Ptr<const Packet>> callback)
{
  m_rxCallback = callback;
}

void
UdpServer::SetRxWithAddressesCallback (Callback<void, Ptr<const Packet>, const Address &, const Address &> callback)
{
  m_rxWithAddressesCallback = callback;
}
  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      if (!m_rxWithAddressesCallback.IsNull ())
      {
        m_rxWithAddressesCallback (packet, from, localAddress);
      }
      if (packet->GetSize () > 0)
        {
