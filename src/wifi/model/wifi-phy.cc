...
void 
WifiPhy::SetReceiveDropCallback (RxDropCallback callback)
{
  m_rxDropCallback = callback;
}

void
WifiPhy::SetTransmitBeginCallback (TxBeginCallback callback)
{
  m_txBeginCallback = callback;
}

void
WifiPhy::SetTransmitEndCallback (PacketCallback callback)
{
  m_txEndCallback = callback;
}

void
WifiPhy::SetTransmitDropCallback (PacketCallback callback)
{
  m_txDropCallback = callback;
}

void
WifiPhy::SetReceiveBeginCallback (RxBeginCallback callback)
{
  m_rxBeginCallback = callback;
}

void
WifiPhy::SetReceiveEndCallback (PacketCallback callback)
{
  ...
  for (auto const& psdu : psdus)
    {
	  ...
	  if (!m_txBeginCallback.IsNull ())
          {
              m_txBeginCallback (mpdu->GetProtocolDataUnit (), txPowerW);
          }
        }
    }
}
      ...
      for (auto& mpdu : *PeekPointer (psdu.second))
        {
          ...
          if (!m_txEndCallback.IsNull ())
      	  {
        	m_txEndCallback (mpdu->GetProtocolDataUnit ());
          }
        }
    }
}
  ...
  for (auto& mpdu : *PeekPointer (psdu))
    {
      ...
      if (!m_txDropCallback.IsNull ())
      {
        m_txDropCallback (mpdu->GetProtocolDataUnit ());
      }
    }
}
  ...
  if (psdu)
    {
	  ...
	  if (!m_rxBeginCallback.IsNull ())
          {
             m_rxBeginCallback (mpdu->GetProtocolDataUnit (), rxPowersW);
          }
        }
    }
}
      ...
      for (auto& mpdu : *PeekPointer (psdu))
        {
          ...
          if (!m_rxEndCallback.IsNull ())
          {
              m_rxEndCallback (mpdu->GetProtocolDataUnit ());
          }
        }
    }
}
      ...
      for (auto& mpdu : *PeekPointer (psdu))
        {
          ...
          if (!m_rxDropCallback.IsNull ())
          {
              m_rxDropCallback (mpdu->GetProtocolDataUnit (), reason);
          }
        }
    }
}
                             ...
                             Time relativeMpduStart, Time mpduDuration)
{
  ...
  // std::cout << "WifiPhy Signal " << signalNoise.signal << std::endl;
  // std::cout << "WifiPhy noise " << signalNoise.noise << std::endl;
