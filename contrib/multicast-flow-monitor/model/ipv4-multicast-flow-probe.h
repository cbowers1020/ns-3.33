...
private:
    ...
    Ptr<Ipv4MulticastFlowClassifier> m_classifier; //!< the Ipv4FlowClassifier this probe is associated with
    Ptr<Ipv4L3Protocol> m_ipv4; //!< the Ipv4L3Protocol this probe is bound to
    std::map<Ipv4Address, std::vector<uint32_t>> m_flowGroupNodes;
};
